// var head = document.getElementById ("LEDHead");
// var text = head.textContent;
// head.textContent = "";
// var textArr = [];
// var frames = 0;
// for (var i = 0; i < text.length; i++) {
//     textArr.add (document.createTextNode (text[i]));
//     head.appendChild (textArr[i]);
// }
...
//     }
// }
var getTag = document.getElementById('LEDHead');

function getTxt(){
    var textOptions = ["LED Project"];
    var chooseOne = Math.floor(Math.random() * textOptions.length);
    var breakDown = textOptions[chooseOne].split('');
...
function randomColor(){
    ...
    for(k=0; k<browserPrefix.length; k++){
        var tagParts1 = ['#LEDHead span{', '-'+browserPrefix[k]+'-animation-duration:6s;-'+browserPrefix[k]+'-animation-timing-function:ease;-'+browserPrefix[k]+'-animation-iteration-count:infinite;', '}'];
        var partsTogether1;
        if(k == 0){
    ...
    for(i=0; i<colorCount; i++){
        for(j=0; j<browserPrefix.length; j++){
            var tagParts2 = ['#LEDHead span:nth-child('+(i+1)+'){', '-'+browserPrefix[j]+'-animation-name:anim'+(i+1)+';', '}'];
            var partsTogether2;
            if(j == 0){
